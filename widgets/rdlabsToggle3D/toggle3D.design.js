function twxToggle3D() {
  return {
    elementTag: 'twx-toggle3D',
 ...
 allowedPatterns: ['.png', '.jpg', '.svg', '.jpeg', '.gif','.bmp'],
         default: '../../extensions/images/toggleOn.png',
 isBindingTarget: true,
      isVisible : false
      },
      {
 ...
 allowedPatterns: ['.png', '.jpg', '.svg', '.jpeg', '.gif','.bmp'],
  resource_image: true,
         default: '../../extensions/images/toggleOff.png',
 isBindingTarget: true,
      isVisible : false
      },
      {
 ...
 isBindingTarget: true,
    ...
    dependencies: {
      files         : ['js/toggle3D-ng.js', 'images/**'],
      angularModules: ['toggle3D-ng']
    },

    designTemplate: function () {
      return '<twx-dt-image id="#widgetId#" src="/extensions/images/3D Toggle.png" opacity="1" hidden="false" width="{{me.width}}" height="{{me.height}}" sx="1" sy="1" sz="1" x="{{me.x}}" y="{{me.y}}" z="{{me.z}}" rx="{{me.rx}}" ry="{{me.ry}}" rz="{{me.rz}}" billboard="{{me.billboard}}" occlude="{{me.occlude}}" decal="{{me.decal}}" shader="{{me.shader}}" istracked="{{me.istracked}}" trackingindicator="{{me.trackingIndicator}}" stationary="{{me.stationary}}"></twx-dt-image>';
    ...
    runtimeTemplate: function (props, twxWidgetEl, fullOriginalDoc, $, projectSettings) {
      ...
      var ctrl = '<twx-dt-3dbutton id="' + props.widgetId + '" class="ng-hide toggle3dWidget ' + props.class + 
      '" text="" ' +  // set to "" so that we can render the text into the button label direclty  //'" text="{{me.pressed ? me.text: me.textnotpressed}}" ' + 
...
twxAppBuilder.widget('twxToggle3D', twxToggle3D);
